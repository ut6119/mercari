...
最短は `GAS WebアプリをAPIとして使うモード` で、ほぼ設定なしで `http://localhost:3000` で開けます。
（将来は `Google Sheets API直結モード` に切替可能）

詳細手順:
- [modern-webapp/README.md](/Users/ookuboyuuta/Documents/New%20project/mercari_inventory_webapp/modern-webapp/README.md)

## GitHub Pages 公開（Google以外のURL）

`main` に push すると `.github/workflows/deploy-pages.yml` が `modern-webapp/public` を自動公開します。  
URL は通常 `https://ut6119.github.io/mercari/` です。
...
- `CLASPRC_JSON_B64`（推奨）: ローカルの `~/.clasprc.json` を base64 化した文字列
- `CLASPRC_JSON`（代替）: ローカルの `~/.clasprc.json` の中身そのまま
- `GAS_DEPLOYMENT_ID`（任意）: 固定URLを維持したいときのデプロイID

macOSなら以下で値をクリップボードへコピーできます。

- `npm run copy:secret:clasprc`
- `npm run copy:secret:clasprc:b64`（推奨）
